# Python script for plotting kNN-SAA, ER-SAA+kNN, ER-SAA+OLS, and PP+OLS for omega = 1
import numpy as np
...
num_sample_sizes = 3
sample_sizes = np.array([[20, 80, 400], [55, 220, 1100], [505, 2020, 10100]], dtype=np.int32)
conf_level = 0.99
...
ERSAA_samp_num = np.array([[2, 3, 4], [5, 6, 7], [5, 6, 7]], dtype=np.int32)

for cov_dim in range(num_covariate_dimensions):
	for samp_size in range(num_sample_sizes):
		for deg in range(num_degrees):
			for data_rep in range(num_data_replicates):
				with open("case" + str(1+cov_dim) + "_ersaa/mod_1/deg_" + str(deg+1) + "/rep_" + str(data_rep+1) + "/ols/samp_" + str(ERSAA_samp_num[cov_dim][samp_size]+1) + "/ersaa_obj.txt") as ERSAA_ols_file:
...
ERSAA_knn_data = np.zeros((num_covariate_dimensions, num_degrees, num_data_replicates, num_sample_sizes, num_saa_replicates))
ERSAA_knn_ucb = np.zeros((num_covariate_dimensions, num_degrees, num_data_replicates, num_sample_sizes))
...
for cov_dim in range(num_covariate_dimensions):
	for samp_size in range(num_sample_sizes):
		for deg in range(num_degrees):
			for data_rep in range(num_data_replicates):
				with open("case" + str(4+cov_dim) + "_ersaa/mod_1/deg_" + str(deg+1) + "/rep_" + str(data_rep+1) + "/kNN/samp_" + str(ERSAA_samp_num[cov_dim][samp_size]+1) + "/ersaa_obj.txt") as ERSAA_knn_file:
					ERSAA_knn_list = ERSAA_knn_file.read().splitlines() 
					ERSAA_knn_data[cov_dim,deg,data_rep,samp_size,:] = [float(i) for i in ERSAA_knn_list]
					ERSAA_knn_gap = ERSAA_knn_data[cov_dim,deg,data_rep,samp_size,:] - SAA_data[deg,data_rep,:]
					ERSAA_knn_gap_mean = np.mean(ERSAA_knn_gap)
					ERSAA_knn_gap_var = np.var(ERSAA_knn_gap)
					ERSAA_knn_ucb[cov_dim,deg,data_rep,samp_size] = (100.0/np.abs(SAA_mean[deg,data_rep]))*(ERSAA_knn_gap_mean + t_value*np.sqrt(ERSAA_knn_gap_var/num_saa_replicates))
				ERSAA_knn_file.close()


# Next, load the BK data with kNN regression
BK_data = np.zeros((num_covariate_dimensions, num_degrees, num_data_replicates, num_sample_sizes, num_saa_replicates))
BK_ucb = np.zeros((num_covariate_dimensions, num_degrees, num_data_replicates, num_sample_sizes))
BK_samp_num = np.array([[2, 3, 4], [2, 3, 4], [2, 3, 4]], dtype=np.int32)

for cov_dim in range(num_covariate_dimensions):
	for samp_size in range(num_sample_sizes):
		for deg in range(num_degrees):
			for data_rep in range(num_data_replicates):
				with open("case" + str(1+cov_dim) + "_bk/mod_1/deg_" + str(deg+1) + "/rep_" + str(data_rep+1) + "/samp_" + str(BK_samp_num[cov_dim][samp_size]+1) + "/bk_obj.txt") as BK_file:
					BK_list = BK_file.read().splitlines() 
					BK_data[cov_dim,deg,data_rep,samp_size,:] = [float(i) for i in BK_list]
					BK_gap = BK_data[cov_dim,deg,data_rep,samp_size,:] - SAA_data[deg,data_rep,:]
					BK_gap_mean = np.mean(BK_gap)
					BK_gap_var = np.var(BK_gap)
					BK_ucb[cov_dim,deg,data_rep,samp_size] = (100.0/np.abs(SAA_mean[deg,data_rep]))*(BK_gap_mean + t_value*np.sqrt(BK_gap_var/num_saa_replicates))
				BK_file.close()


# Next, load the Point Prediction data with OLS regression
PP_ols_data = np.zeros((num_covariate_dimensions, num_degrees, num_data_replicates, num_sample_sizes, num_saa_replicates))
PP_ols_ucb = np.zeros((num_covariate_dimensions, num_degrees, num_data_replicates, num_sample_sizes))
PP_samp_num = np.array([[2, 3, 4], [2, 3, 4], [2, 3, 4]], dtype=np.int32)

for cov_dim in range(num_covariate_dimensions):
	for samp_size in range(num_sample_sizes):
		for deg in range(num_degrees):
			for data_rep in range(num_data_replicates):
				with open("case" + str(1+cov_dim) + "_pointpred/mod_1/deg_" + str(deg+1) + "/rep_" + str(data_rep+1) + "/ols/samp_" + str(PP_samp_num[cov_dim][samp_size]+1) + "/pointpred_obj.txt") as PP_ols_file:
					PP_ols_list = PP_ols_file.read().splitlines() 
					PP_ols_data[cov_dim,deg,data_rep,samp_size,:] = [float(i) for i in PP_ols_list]
					PP_ols_gap = PP_ols_data[cov_dim,deg,data_rep,samp_size,:] - SAA_data[deg,data_rep,:]
					PP_ols_gap_mean = np.mean(PP_ols_gap)
					PP_ols_gap_var = np.var(PP_ols_gap)
					PP_ols_ucb[cov_dim,deg,data_rep,samp_size] = (100.0/np.abs(SAA_mean[deg,data_rep]))*(PP_ols_gap_mean + t_value*np.sqrt(PP_ols_gap_var/num_saa_replicates))
				PP_ols_file.close()


# Plot PP+OLS vs ER-SAA+OLS vs ER-SAA+kNN vs kNN-SAA
box_colors = ['black', 'limegreen', 'blue', 'red']
plot_y_max = [12.0, 8.0, 30.0]
for deg in range(num_degrees):
	for cov in range(num_covariate_dimensions):
		...
		for meth in range(4):
			...
			for samp_size in range(num_sample_sizes):
				if meth == 0:
					data[:,samp_size] = BK_ucb[cov,deg,:,samp_size]
					box_positions[samp_size] = 1 + 2.5*samp_size
				elif meth == 1:
					data[:,samp_size] = ERSAA_knn_ucb[cov,deg,:,samp_size]
					...
					data[:,samp_size] = PP_ols_ucb[cov,deg,:,samp_size]
			...
			plt.savefig("/Users/rohitkannan/Desktop/OPRE Revision/test_cases_v2/figures/fig2_cov" + str(cov+1) + "_deg" + str(deg+1) + ".eps", format='eps')
