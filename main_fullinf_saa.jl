...
#*========= MODEL INFORMATION ==========
const caseNum = 1
const paramsFile = "params_case" * string(caseNum) * "_fullinf_saa.jl"
#*======================================
...
const baseDirName = "case" * string(caseNum) * "_fullinf_saa/" * "mod_" * string(modRepNum) * "/" * "deg_" * string(degNum) * "/"
...
const fullinfSAAObjFile = "fullinf_saa_obj.txt"
const fullinfSAATimeFile = "fullinf_saa_time.txt"
...
demand_scen_MC = generateTrueCondScenarios(numMCScenarios,covariate_obs,degree[degNum],coeff_true)
...
if(storeResults)
	...
	obj_est_file = subDirName2 * fullinfSAAObjFile
	open(obj_est_file, "w") do f
		write(f,"$SAAObj")
	end	
	
	time_obj_file = subDirName2 * fullinfSAATimeFile
